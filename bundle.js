...
if (isNode()) {
  ...
  fetch = promisify(fs.readFile)
}

function fetchAndInstantiate(url, importObject) {
  return fetch(url).then(response =>
    (response.arrayBuffer ? response.arrayBuffer() : response)
  ).then(bytes =>
...
if (isNode()) {
  ...
  bundle.fetchAndInstantiate = fetchAndInstantiate;
  bundle.copyCStr = copyCStr;
  bundle.getStr = getStr;
  bundle.newString = newString;
}
